function pull_stats_airxbcal_merraTseries(year, filter);
...
trace.RunDate = datetime('now','TimeZone','local','Format', ...
                         'd-MMM-y HH:mm:ss Z');
trace.Reason = 'MERRA First of month timeseris test with AIRXBCAL';
trace.klayers = false;
...
for month = 1:12 
    for dom = 1:1   % collect stats for first three days of month
       ...
       dayfiles(end+1) = {fullfile(basedir, ...
                               sprintf('merra_airxbcal_day%03s_clear.rtp',st))};
    end
end
...
eval_str = ['save ' startdir '/rtp_airxbcal_ts_merra_rad_'  int2str(year) ...
