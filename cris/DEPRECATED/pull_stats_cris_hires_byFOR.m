function pull_stats_cris_hires_byFOR(year, filter, ifor, cfg);
...
rtpdir = '/asl/rtp/rtp_cris_PLtest_PLon/';
if bCheckConfig & isfield(cfg, 'rtpdir')
    rtpdir = cfg.rtpdir;
end

statsdir = '/asl/data/stats/cris_PL';
...
dayfiles = dir(fullfile(basedir, sprintf('cris_PLtest_PLon_d*_%s.rtp',sSubset)));
...
for giday = 1:length(dayfiles)
   ...
   a = dir(fullfile(dayfiles(giday).folder,dayfiles(giday).name));
   if a.bytes < 100000
       fprintf(2, '**>> ERROR: short input rtp file %s\n', dayfiles(giday).name); 
       continue;
   end

      [h,ha,p,pa] = rtpread(fullfile(dayfiles(giday).folder,dayfiles(giday).name));
      f = h.vchan;
    ...
    switch filter
      ...
      infor = find(p.xtrack == ifor);
      keep_inds = intersect(k, infor);
      pp = rtp_sub_prof(p, keep_inds);
      clear p
...
eval_str = sprintf(['save /asl/data/stats/cris_PL/' ...
                    'rtp_cris_hires_PL_on_%s_%s_FOR%d_%s robs rclr ' ...
                    'rbias_std *_mean count trace latbinedges'], int2str(year),sSubset,ifor,sDescriptor);
