function fit_robust_series_plat(lati)
...
anom = zeros(1,4214,2378);
all_bcorr = zeros(1,2378,10,10);
x1 = zeros(4214,1)*NaN;

% restore mission aggregate stats
% $$$ load('~/testoutput/save_test.mat')
fprintf(1, '>>> Reading in concatenated mission stats\n')
tStart = tic;
load('~/testoutput/rates_temp/save_rand.mat')
tElapsed = toc(tStart);
fprintf(1, '>>> Input data read in %d seconds\n', tElapsed);
...
warning('off','all');
    fprintf(1, '>>> Processing latitude bin %d\n', lati);
...
%y=squeeze(btcal(it,lati,i))-squeeze(btobs(it,lati,i));
        ...
        all_b(1,i,:) = b;
        all_rms(1,i) = stats.s;
        x1 = x1*NaN;  % reset x1 for next set of stats
        x1(ii) = stats.resid;
        anom(1,:,i) = x1;
...
outfile = fullfile('/home/sbuczko1/testoutput/rates_temp', ['rates_' int2str(lati) '.mat']);
save(outfile, 'rates_clr', 'rates_clr_err', 'ig')
tElapsed = toc(tStart);
fprintf(1, ['>>> Finished processing latitude bin %d. Elapsed runtime: ' ...
            '%d seconds\n'], lati, tElapsed)
