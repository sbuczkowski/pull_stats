function pull_stats_airicrad_clear_land(year, filter, cfg);
...
statsdir = '/asl/data/stats/airs/clear_land';
if bCheckConfig & isfield(cfg, 'statsdir')
    statsdir = cfg.statsdir;
end

basedir = fullfile(rtpdir, 'clear_land', int2str(year));
dayfiles = dir(fullfile(basedir, 'era_airicrad_day*_clear_land.rtp'));
...
outfile = fullfile(statsdir, sprintf('rtp_airicrad_era_rad_kl_%s_clear_land_%s', ...
