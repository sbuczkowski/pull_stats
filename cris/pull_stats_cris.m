...
statsdir = sprintf('/asl/data/stats/%s', instName);
...
basedir = fullfile(rtpsrcdir, sprintf('%s', descriptor), int2str(year));
dayfiles = dir(fullfile(basedir, sprintf('%s_%s_%s_d*_%s.rtp', ...
                                         instName, fscale, model, descriptor));
   ...
   if a.bytes > 100000
      ...
      for ilat = 1:nlatbins-1
          ...
          for z = 1:9  % loop over FOVs to further sub-select
              ...
              r = box_to_ham(p2.robs1);  % assumes r in freq order!!  Needed
                                  % for lowres

              robs(iday,ilat,z,:) = nanmean(r,2);
              rcal(iday,ilat,z,:) = nanmean(p2.rclr,2);
              rclrbias_std(iday, ilat,:) = nanstd(r-p2.rclr,0,2);

              if ~strcmp(descriptor, 'clear')
                  rcld(iday,ilat,z,:) = nanmean(p2.rcld,2);
                  rcldbias_std(iday, ilat,:) = nanstd(r-p2.rcld,0, ...
                                                      2);
              end
